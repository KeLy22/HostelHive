...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_room_change'])) {
    ...
    $stmt->execute([$student_id, $student['room_id'], $requested_room_id, $reason]);
...
.sidebar {
    width: 230px; background-color: #2c3e50;
    position: fixed; top: 0; left: 0;
    height: 100%; color: white; padding-top: 30px;
}
...
<div class="main-content">
    ...
    <div class="card p-3 my-3">
        ...
        <?php elseif ($request && $request['status'] == 'pending'): ?>
            <h5>Room Request Pending</h5>
            <hr>
            <p>Your request for Room ID <strong><?= htmlspecialchars($request['requested_room_id']); ?></strong> is <span class="badge bg-warning text-dark">Pending</span>.</p>
    ...
    <div class="card p-3 my-3">
        ...
        <form method="POST">
            <div class="mb-2">
                <label>Requested Room ID</label>
                <input type="number" name="requested_room_id" class="form-control" placeholder="Enter room ID" required>
            </div>
            <div class="mb-2">
                <label>Reason</label>
